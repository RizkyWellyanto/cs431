...
int main(){
  ...
  pid_controller_init(&controller_y, 290, 1500, 0.02, KP_Y, KI_Y, KD_Y);
  ...
  while (1) {
    ...
    if(hz50_scaler == 0) {
      ...
      // between 900-2100 us
      setMotorDuty(MOTOR_X_CHAN, pidX_controller(Xpos));
      setMotorDuty(MOTOR_Y_CHAN, pidY_controller(Ypos));
    }
